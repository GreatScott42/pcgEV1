...
public static class HeightmapGenerator
{
    ...
    public static float[,] GenerateRandomNoise(int resolution, int seed)
    {
        ...
        for (int i=0;i<resolution;i++)
        {
            ...
            {
                heights[i, j] = UnityEngine.Random.Range(0, 2);
            }
        }
        ...
        for (int y = 0; y < resolution; y++)
        {
            for (int x = 0; x < resolution; x++)
            {
                ...
                if (interpolationMode==InterpolationMode.Bilinear)
                {
                    ...
                    //int nearestY = Mathf.RoundToInt(y / (float)latticeSpacing);
                    int nearestX = Mathf.Min(x / latticeSpacing, numberOfCells - 1);
                    int nearestY = Mathf.Min(y / latticeSpacing, numberOfCells - 1);

                    //float tx = (x - nearestX * latticeSpacing) / (float)latticeSpacing;
                    //float ty = (y - nearestY * latticeSpacing) / (float)latticeSpacing;
                    //?
                    //nearestX = Mathf.Clamp(nearestX, 0, controlPointResolution - 1);
                    //nearestY = Mathf.Clamp(nearestY, 0, controlPointResolution - 1);
                    ...
                    //??????????????vvvv
                    float tx = (x - nearestX*latticeSpacing) / (float)latticeSpacing;
                    //float ty = (y - latticeSpacing * nearestY) / latticeSpacing;
                    float ty = (y - nearestY*latticeSpacing) / (float)latticeSpacing;

                    float topl = controlPoints[nearestY, nearestX];
                    float topr = controlPoints[nearestY, nearestX+1];

                    float bottoml = controlPoints[nearestY+1, nearestX];
                    float bottomr = controlPoints[nearestY+1, nearestX + 1];


                    //float top = Mathf.Lerp(topl, topr,tx);
                    float top = LinearInterpolation(topl,topr,tx);

                    //float bottom = Mathf.Lerp(bottoml, bottomr, tx);
                    float bottom = LinearInterpolation(bottoml,bottomr,tx);

                    //float f = Mathf.Lerp(top, bottom, ty);
                    float f = LinearInterpolation(top,bottom,ty);
                ...
                if (interpolationMode == InterpolationMode.Bicubic)
                {
                    ...
                    //float top = Mathf.Lerp(topl, topr,tx);
                    float top = LinearInterpolation(topl, topr, BicubicWeight(tx));

                    //float bottom = Mathf.Lerp(bottoml, bottomr, tx);
                    float bottom = LinearInterpolation(bottoml, bottomr, BicubicWeight(tx));

                    //float f = Mathf.Lerp(top, bottom, ty);
                    float f = LinearInterpolation(top, bottom, BicubicWeight(ty));
    ...
    private static float[,] GenerateControlPoints(int resolution, int seed)
    {
        ...
        for (int i = 0; i < resolution; i++)
        {
            for (int j = 0; j < resolution; j++)
            {
                points[i, j] = UnityEngine.Random.Range(0, 2);
            }
        }
