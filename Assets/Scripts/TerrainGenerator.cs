...
public class TerrainGenerator : MonoBehaviour
{
    ...
    public void gentree()
    {
        float[,] heights = heights = HeightmapGenerator.GenerateValueNoise(
                    65,
                    latticeSpacing,
                    seed,
                    valueNoiseInterpolation
        ...
        for (int x = 0; x < 65; x++)
        {
            for (int y = 0; y < 65; y++)
            {
                //heights[x, y] = Mathf.Abs(Mathf.Sin(Time.time) * heights[x, y]);
                //heights[x, y] = Mathf.Abs(Mathf.Sin(Time.time) * heights[x, y]);
            }
        }
        generatedTerrain.terrainData.SetHeights(0, 0, heights);
