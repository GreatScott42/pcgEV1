...
public static class PerlinNoiseGenerator
{
        ...
        HeightmapGenerator.InterpolationMode interpolationMode)
    {
        ...
        Vector2Int topl = new Vector2Int(x1,y1);
        Vector2Int topr = new Vector2Int(x2,y1);

        Vector2Int bottoml = new Vector2Int(x1,y2);
        Vector2Int bottomr = new Vector2Int(x2,y2);
        ...
        Vector2 grtopl = GetGradient(topl.x,topl.y,seed);
        Vector2 grtopr = GetGradient(topr.x,topr.y,seed);
        Vector2 grbottoml = GetGradient(bottoml.x,bottoml.y,seed);
        Vector2 grbottomr = GetGradient(bottomr.x,bottomr.y,seed);
        // 4. Crear los cuatro vectores de desplazamiento.
        Vector2 dtopl = new Vector2(localx,localy);
        Vector2 dtopr = new Vector2(localx-1, localy);
        Vector2 dbottoml = new Vector2(localx,localy-1);
        Vector2 dbottomr = new Vector2(localx-1,localy-1);
        // 5. Calcular el producto punto para cada esquina.
        float dot00 = Vector2.Dot(grtopl,dtopl);
        float dot01 = Vector2.Dot(grtopr,dtopr);
        float dot10 = Vector2.Dot(grbottoml,dbottoml);
        float dot11 = Vector2.Dot(grbottomr,dbottomr);
        // 6. Obtener weightX y weightY según el modo de interpolación.
        float r=0;

        if (interpolationMode == HeightmapGenerator.InterpolationMode.Bilinear)
        {
            float top = LinearInterpolation(dot00, dot01, localx);
            float bottom = LinearInterpolation(dot10, dot11, localx);

            r = LinearInterpolation(top, bottom, localy);
        }else if (interpolationMode==HeightmapGenerator.InterpolationMode.Bicubic)
        {
            float top = LinearInterpolation(dot00, dot01, BicubicWeight(localx));
            float bottom = LinearInterpolation(dot10, dot11, BicubicWeight(localx));

            r = LinearInterpolation(top, bottom, BicubicWeight(localy));
        }
