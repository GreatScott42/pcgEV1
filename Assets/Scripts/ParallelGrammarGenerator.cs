...
public class ParallelGrammarGenerator : MonoBehaviour
{
        ...
        List<string> derivation = null)
    {
        ...
        {
            foreach (char a in axiom)
            {
                foreach (LSystemRule rule in rules)
                {
                    Debug.Log(rule.predecessor+"/"+a);
                    string aa = a.ToString();
                    if (rule.predecessor.Equals(aa))
                    {
                        Debug.Log("rule predecessor: "+rule.predecessor);
                        newAxiom += rule.successor;
                    }
                }
            }
            Debug.Log(newAxiom+"<----AXIOMA");
            axiom = newAxiom;
            derivation.Add(axiom);
            newAxiom = "";
        }
