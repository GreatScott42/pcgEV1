...
public static class DiamondSquareGenerator
{
    ...
    public static float[,] GenerateHeightmap(
        ...
        while (stepSize > 1)
        {
            ...
            //
            Debug.Log(heights[resolution-1, resolution - 1]);
        ...
        System.Random random)
    {
        ...
        // 
        for (int i=halfStep;i<resolution;i+=stepSize)
        {
            for (int j = halfStep; j < resolution; j += stepSize)
            {
                float topl = heights[i-halfStep, j-halfStep];
                float topr = heights[i-halfStep,j+halfStep];
                float bottoml = heights[i+halfStep, j-halfStep];
                float bottomr = heights[i+halfStep, j+halfStep];

                float x = (topl + topr + bottoml + bottomr) / 4;

                //random offset
                x+=RandomOffset(random,roughness);
                //x+= 0.2f*stepSize;
                x = Mathf.Clamp01(x);
        ...
        System.Random random)
    {
        ...
        for (int i=0;i<resolution;i+=halfStep)
        {
            ...
            for (int j=x;j<resolution;j+=halfStep)
            {
                float suma=0;
                int contador = 0;
                ...
                prom += RandomOffset(random,roughness);
                //Debug.Log(RandomOffset(random, roughness));
                //prom += 0.2f;
                prom = Mathf.Clamp01(prom);
                heights[i,j] = prom;
            }
        }
